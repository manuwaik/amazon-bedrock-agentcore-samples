...
Each user_id gets an isolated S3 prefix for session isolation.

Flow: restore from S3 -> resume session -> query -> save to S3
"""
...
def restore_sessions(s3, namespace: str):
    """Download session files from S3 to ~/.claude/."""
    prefix = f"{namespace}/.claude/"
...
def save_sessions(s3, namespace: str):
...
def _session_id_file(namespace: str) -> Path:
    return CLAUDE_HOME / f".session_id_{namespace}"


def load_session_id(namespace: str) -> str | None:
    f = _session_id_file(namespace)
    if f.exists():
...
def save_session_id(namespace: str, sid: str):
    _session_id_file(namespace).parent.mkdir(parents=True, exist_ok=True)
    _session_id_file(namespace).write_text(sid)
...
async def invoke(payload):
    namespace = payload.get("user_id", "default")
    s3 = boto3.client("s3")

    # Restore session state from S3
    restore_sessions(s3, namespace)
    ...
    previous_session = load_session_id(namespace)
    ...
    async for message in query(prompt=payload.get("prompt", "Hello!"), options=options):
        ...
        if isinstance(message, ResultMessage) and message.session_id:
            save_session_id(namespace, message.session_id)
        yield message

    # Persist session state to S3
    save_sessions(s3, namespace)
